...
import { 
  ...
  insertFormFolderSchema, insertFormSchema, insertFormSubmissionSchema, insertFormViewSchema, insertFormAnalyticsSchema,
  ...
  formFolders, forms, formSubmissions, formViews, formAnalytics
} from "@shared/schema";
import { z } from "zod";
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get("/api/form-folders", async (req, res) => {
    try {
      const folders = await db.select().from(formFolders).orderBy(asc(formFolders.order));
      res.json(folders);
    } catch (error) {
  ...
  app.post("/api/form-folders", async (req, res) => {
    try {
      const validatedData = insertFormFolderSchema.parse({
        ...req.body,
        createdBy: "e56be30d-c086-446c-ada4-7ccef37ad7fb" // Default user ID
      });
      
      const folder = await storage.createFormFolder(validatedData);
      ...
      await createAuditLog(
        ...
        folder.name,
        undefined,
        `Created form folder: ${folder.name}`,
        null,
        folder,
        req
      );
  ...
  app.delete("/api/form-folders/:id", async (req, res) => {
    try {
      const folder = await storage.getFormFolderById(req.params.id);
      if (!folder) {
        return res.status(404).json({ message: "Form folder not found" });
      }
      
      await storage.deleteFormFolder(req.params.id);
      ...
      await createAuditLog(
        ...
        folder.name,
        undefined,
        `Deleted form folder: ${folder.name}`,
        folder,
        null,
        req
      );
  ...
  app.post("/api/forms", async (req, res) => {
    try {
      const validatedData = insertFormSchema.parse({
        ...req.body,
        createdBy: "e56be30d-c086-446c-ada4-7ccef37ad7fb", // Default user ID
        updatedBy: "e56be30d-c086-446c-ada4-7ccef37ad7fb"
      });
  ...
  app.get("/api/form-submissions/:id", async (req, res) => {
    try {
      const submissionResult = await db.select()
        .from(formSubmissions)
        .where(eq(formSubmissions.id, req.params.id))
        .limit(1);
  ...
  app.post("/api/form-submissions", async (req, res) => {
    try {
      ...
      const submissionResult = await db.insert(formSubmissions).values(validatedData).returning();
      const submission = submissionResult[0];
  ...
  app.get("/api/form-analytics", async (req, res) => {
    try {
      const { formId, from, to } = req.query as { formId?: string; from?: string; to?: string };
      
      let query = db.select().from(formAnalytics);
      
      if (formId && formId !== "all") {
        query = query.where(eq(formAnalytics.formId, formId));
      }
      
      if (from && to) {
        query = query.where(and(
          sql`${formAnalytics.date} >= ${from}`,
          sql`${formAnalytics.date} <= ${to}`
        ));
      }
      
      const analytics = await query.orderBy(desc(formAnalytics.date));
      res.json(analytics);
    } catch (error) {
  ...
  app.post("/api/form-views", async (req, res) => {
    try {
      const validatedData = insertFormViewSchema.parse(req.body);
      
      const viewResult = await db.insert(formViews).values(validatedData).returning();
      ...
      res.status(500).json({ message: "Failed to create form view" });
    }
  });
