...
import { 
  ...
  insertClientBundleSchema, insertBundleTaskTemplateSchema,
  ...
  roles, permissions, userRoles, notificationSettings, clientProducts, clientBundles, productBundles, bundleProducts, bundleTaskTemplates,
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/objects/upload", async (req, res) => {
    try {
      const { entityType = "profile", entityId = "default", fileExtension = ".jpg" } = req.body;
      const { ObjectStorageService } = await import("./objectStorage");
      const objectStorageService = new ObjectStorageService();
      const uploadURL = await objectStorageService.getObjectEntityUploadURL(entityType, entityId, fileExtension);
      res.json({ uploadURL });
    } catch (error) {
  ...
  // Client Bundle Assignment Routes - Track which bundles clients have purchased
  app.get("/api/clients/:clientId/bundles", async (req, res) => {
    try {
      const clientBundleAssignments = await db.select({
        id: clientBundles.id,
        bundleId: clientBundles.bundleId,
        projectId: clientBundles.projectId,
        status: clientBundles.status,
        startDate: clientBundles.startDate,
        endDate: clientBundles.endDate,
        lastTaskGeneration: clientBundles.lastTaskGeneration,
        bundle: {
      ...
      .from(clientBundles)
      .innerJoin(productBundles, eq(clientBundles.bundleId, productBundles.id))
      .where(eq(clientBundles.clientId, req.params.clientId));

      res.json(clientBundleAssignments);
    } catch (error) {
      console.error("Error fetching client bundles:", error);
      res.status(500).json({ message: "Failed to fetch client bundles" });
    }
  });

  app.post("/api/clients/:clientId/bundles", async (req, res) => {
    try {
      const validatedData = insertClientBundleSchema.parse({
        ...req.body,
        clientId: req.params.clientId
      });

      const [newClientBundle] = await db.insert(clientBundles)
        .values(validatedData)
        .returning();
      ...
      res.status(500).json({ message: "Failed to assign bundle to client" });
    }
  });

  app.delete("/api/clients/:clientId/bundles/:bundleAssignmentId", async (req, res) => {
    try {
      await db.delete(clientBundles)
        .where(and(
          eq(clientBundles.id, req.params.bundleAssignmentId),
          eq(clientBundles.clientId, req.params.clientId)
        ));

      res.status(204).send();
    } catch (error) {
      console.error("Error removing client bundle:", error);
      res.status(500).json({ message: "Failed to remove bundle from client" });
    }
  });

  // Bundle Task Template Routes - Define what tasks each bundle creates
  app.get("/api/bundles/:bundleId/task-templates", async (req, res) => {
    try {
      const templates = await db.select()
        .from(bundleTaskTemplates)
        .where(eq(bundleTaskTemplates.bundleId, req.params.bundleId))
        .orderBy(asc(bundleTaskTemplates.dayOffset));

      res.json(templates);
    } catch (error) {
      console.error("Error fetching bundle task templates:", error);
      res.status(500).json({ message: "Failed to fetch bundle task templates" });
    }
  });

  app.post("/api/bundles/:bundleId/task-templates", async (req, res) => {
    try {
      const validatedData = insertBundleTaskTemplateSchema.parse({
        ...req.body,
        bundleId: req.params.bundleId
      });

      const [newTemplate] = await db.insert(bundleTaskTemplates)
        .values(validatedData)
        .returning();

      res.status(201).json(newTemplate);
    } catch (error) {
      console.error("Error creating bundle task template:", error);
      if (error instanceof z.ZodError) {
        return res.status(400).json({ message: "Invalid data", errors: error.errors });
      }
      res.status(500).json({ message: "Failed to create bundle task template" });
    }
  });

  app.put("/api/bundles/:bundleId/task-templates/:templateId", async (req, res) => {
    try {
      const validatedData = insertBundleTaskTemplateSchema.partial().parse(req.body);

      const [updatedTemplate] = await db.update(bundleTaskTemplates)
        .set(validatedData)
        .where(and(
          eq(bundleTaskTemplates.id, req.params.templateId),
          eq(bundleTaskTemplates.bundleId, req.params.bundleId)
        ))
        .returning();
      ...
      console.error("Error updating bundle task template:", error);
      res.status(500).json({ message: "Failed to update bundle task template" });
    }
  });

  app.delete("/api/bundles/:bundleId/task-templates/:templateId", async (req, res) => {
    try {
      await db.delete(bundleTaskTemplates)
        .where(and(
          eq(bundleTaskTemplates.id, req.params.templateId),
          eq(bundleTaskTemplates.bundleId, req.params.bundleId)
        ));
      ...
      console.error("Error deleting bundle task template:", error);
      res.status(500).json({ message: "Failed to delete bundle task template" });
    }
  });

  // Monthly Task Generation API - Generate recurring tasks for active client bundles
  app.post("/api/generate-monthly-tasks", async (req, res) => {
    try {
      const { month, year } = req.body;
      const targetDate = new Date(year, month - 1, 1); // First day of the month
      const userId = req.session?.userId || "e56be30d-c086-446c-ada4-7ccef37ad7fb";

      // Get all active client bundle assignments that need task generation
      const activeClientBundles = await db.select({
        assignmentId: clientBundles.id,
        clientId: clientBundles.clientId,
        projectId: clientBundles.projectId,
        bundleId: clientBundles.bundleId,
        lastTaskGeneration: clientBundles.lastTaskGeneration,
        templates: bundleTaskTemplates
      })
      .from(clientBundles)
      .innerJoin(productBundles, eq(clientBundles.bundleId, productBundles.id))
      .innerJoin(bundleTaskTemplates, eq(productBundles.id, bundleTaskTemplates.bundleId))
      .where(and(
        eq(clientBundles.status, "active"),
        or(
          sql`${clientBundles.lastTaskGeneration} IS NULL`,
          sql`${clientBundles.lastTaskGeneration} < ${targetDate}`
        )
      ));

      const tasksCreated = [];

      for (const bundleData of activeClientBundles) {
        // Create tasks based on templates
        for (let i = 0; i < bundleData.templates.quantity; i++) {
          const dueDate = new Date(targetDate);
          dueDate.setDate(dueDate.getDate() + (bundleData.templates.dayOffset || 0));

          const taskTitle = bundleData.templates.title.replace('#{sequence}', (i + 1).toString());

          const newTask = {
            title: taskTitle,
            description: bundleData.templates.description,
            priority: bundleData.templates.priority,
            clientId: bundleData.clientId,
            projectId: bundleData.projectId,
            assignedTo: bundleData.templates.assignedTo,
            dueDate: dueDate,
        ...
        // Update last task generation timestamp
        await db.update(clientBundles)
          .set({ lastTaskGeneration: new Date() })
          .where(eq(clientBundles.id, bundleData.assignmentId));
      }
      ...
      await createAuditLog(
        ...
        "monthly_tasks",
        `${month}-${year}`,
        `Monthly Tasks ${month}/${year}`,
        userId,
        `Generated ${tasksCreated.length} monthly tasks from bundle templates`,
        null,
        { tasksCount: tasksCreated.length, month, year },
        req
      );
