...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  */
  ...
  // ===== RESOURCES SYSTEM API ENDPOINTS =====
  ...
  app.get("/api/resources/categories", async (req, res) => {
    try {
      const categories = await db.select().from(resourceCategories)
        ...
        .orderBy(asc(resourceCategories.order), asc(resourceCategories.name));
      res.json(categories);
    } catch (error) {
  ...
  app.post("/api/resources/categories", async (req, res) => {
    try {
      const validatedData = insertResourceCategorySchema.parse(req.body);
      const [newCategory] = await db.insert(resourceCategories).values({
        ...validatedData,
      ...
      await createAuditLog(
        ...
        newCategory.name,
        req.session?.userId || "system",
        `Created resource category: ${newCategory.name}`,
        null,
        newCategory,
        req
      );
  ...
  app.put("/api/resources/categories/:id", async (req, res) => {
    try {
      const { id } = req.params;
      const validatedData = insertResourceCategorySchema.parse(req.body);
      
      const [updatedCategory] = await db.update(resourceCategories)
        .set({ ...validatedData, updatedAt: new Date() })
      ...
      await createAuditLog(
        ...
        req.session?.userId || "system",
        `Updated resource category: ${updatedCategory.name}`,
        null,
        updatedCategory,
        req
      );
  ...
  app.delete("/api/resources/categories/:id", async (req, res) => {
    try {
      const { id } = req.params;
      
      // Soft delete by setting isActive to false
      const [deletedCategory] = await db.update(resourceCategories)
      ...
      await createAuditLog(
        ...
        req.session?.userId || "system",
        `Deleted resource category: ${deletedCategory.name}`,
        deletedCategory,
        null,
        req
      );
  ...
  app.get("/api/resources", async (req, res) => {
    try {
      ...
      let query = db.select({
        ...
        isRequired: resources.isRequired,
      ...
      if (category && category !== 'all') {
        query = query.where(eq(resources.categoryId, category as string));
      }

      const resourcesList = await query.orderBy(asc(resources.order), asc(resources.title));
  ...
  app.get("/api/resources/:id", async (req, res) => {
    try {
      ...
      const [progress] = await db.select()
        ...
        .where(and(
          eq(resourceProgress.resourceId, id),
          eq(resourceProgress.userId, userId)
        ));

      // Get prerequisites
      const prerequisites = await db.select({
        id: resourcePrerequisites.id,
        prerequisiteId: resourcePrerequisites.prerequisiteId,
        prerequisiteTitle: resources.title,
        isRequired: resourcePrerequisites.isRequired
      })
      .from(resourcePrerequisites)
      .leftJoin(resources, eq(resourcePrerequisites.prerequisiteId, resources.id))
      .where(eq(resourcePrerequisites.resourceId, id));
  ...
  app.post("/api/resources", async (req, res) => {
    try {
      ...
      await createAuditLog(
        ...
        newResource.title,
        req.session?.userId || "system",
        `Created resource: ${newResource.title}`,
        null,
        newResource,
        req
      );
  ...
  app.put("/api/resources/:id", async (req, res) => {
    try {
      ...
      await createAuditLog(
        ...
        req.session?.userId || "system",
        `Updated resource: ${updatedResource.title}`,
        null,
        updatedResource,
        req
      );
  ...
  app.delete("/api/resources/:id", async (req, res) => {
    try {
      ...
      // Soft delete by setting isActive to false
      const [deletedResource] = await db.update(resources)
        .set({ isActive: false, updatedAt: new Date() })
        .where(eq(resources.id, id))
        .returning();
      ...
      await createAuditLog(
        ...
        req.session?.userId || "system",
        `Deleted resource: ${deletedResource.title}`,
        deletedResource,
        null,
        req
      );
  ...
  app.post("/api/resources/:id/progress", async (req, res) => {
    try {
      ...
      const progressData = {
        ...
        startedAt: existingProgress.length === 0 ? new Date() : undefined,
      ...
      if (existingProgress.length > 0) {
        ...
        [progressRecord] = await db.update(resourceProgress)
          .set(progressData)
          .where(and(
            eq(resourceProgress.resourceId, id),
            eq(resourceProgress.userId, userId)
        ...
        [progressRecord] = await db.insert(resourceProgress)
          .values({
            resourceId: id,
            userId,
            ...progressData
  ...
  app.get("/api/resources/progress", async (req, res) => {
    try {
      const userId = req.session?.userId || "e56be30d-c086-446c-ada4-7ccef37ad7fb";
      ...
      const userProgress = await db.select({
        ...
        timeSpent: resourceProgress.timeSpent,
        completedAt: resourceProgress.completedAt,
        resourceTitle: resources.title,
        resourceType: resources.type
      })
      .from(resourceProgress)
      .leftJoin(resources, eq(resourceProgress.resourceId, resources.id))
  ...
  // Resource Links endpoints (for the table view)
  app.get("/api/resources/links", async (req, res) => {
    try {
      const { category } = req.query;
      let query = db.select({
        id: resourceLinks.id,
        ...
        description: resourceLinks.description,
        categoryId: resourceLinks.categoryId,
        createdAt: resourceLinks.createdAt,
        categoryName: resourceCategories.name
      })
      .from(resourceLinks)
      .leftJoin(resourceCategories, eq(resourceLinks.categoryId, resourceCategories.id))
      .where(eq(resourceLinks.isActive, true));

      if (category && category !== 'all') {
        query = query.where(eq(resourceLinks.categoryId, category as string));
      }

      const links = await query.orderBy(desc(resourceLinks.createdAt));
      res.json(links);
    } catch (error) {
  ...
  app.post("/api/resources/links", async (req, res) => {
    try {
      ...
      const [newLink] = await db.insert(resourceLinks).values({
        ...validatedData,
        createdBy: req.session?.userId || "e56be30d-c086-446c-ada4-7ccef37ad7fb"
      }).returning();
      ...
      await createAuditLog(
        ...
        newLink.title,
        req.session?.userId || "system",
        `Created resource link: ${newLink.title}`,
        null,
        newLink,
        req
      );
  ...
  app.put("/api/resources/links/:id", async (req, res) => {
    try {
      const { id } = req.params;
      const validatedData = insertResourceLinkSchema.parse(req.body);
      
      const [updatedLink] = await db.update(resourceLinks)
        .set({ ...validatedData, updatedAt: new Date() })
      ...
      await createAuditLog(
        ...
        updatedLink.title,
        req.session?.userId || "system",
        `Updated resource link: ${updatedLink.title}`,
        null,
        updatedLink,
        req
      );
      ...
      res.status(500).json({ error: "Failed to update resource link" });
    }
  });

  app.delete("/api/resources/links/:id", async (req, res) => {
    try {
      const { id } = req.params;
      
      // Soft delete by setting isActive to false
      const [deletedLink] = await db.update(resourceLinks)
        .set({ isActive: false, updatedAt: new Date() })
        .where(eq(resourceLinks.id, id))
        .returning();
      ...
      await createAuditLog(
        ...
        deletedLink.title,
        req.session?.userId || "system",
        `Deleted resource link: ${deletedLink.title}`,
        deletedLink,
        null,
        req
      );

      res.json({ message: "Resource link deleted successfully" });
    } catch (error) {
  ...
  app.post("/api/courses", async (req, res) => {
    try {
      ...
      console.log('Creating course with userId:', userId);
      ...
      const validatedData = insertCourseSchema.parse(dataToInsert);
