...
## Recent Changes
- **Implemented Client Hub Calendar Merge Tags** - Added comprehensive merge tag functionality to Client Hub Calendar appointment Title and Description fields, matching the platform's existing Email/SMS merge tag patterns. Features 22 total merge tags organized into 4 categories (Client Information, Staff/Team Information, Appointment Details, System), DropdownMenu UI with search functionality, double curly brace format {{tag}}, and grouped tags with separators. Users can now dynamically insert client data, staff info, appointment details, and system values into calendar appointments.
- **Fixed Workflow Template Duplicate Prevention** - Resolved issue where "Save As Template" was creating duplicate templates. Implemented database-level uniqueness enforcement with NOT NULL UNIQUE constraint on workflow_templates.workflow_id column, preventing multiple templates from the same workflow. Added application-level duplicate check that returns 409 Conflict with clear error message. Enhanced frontend error handling to guide users to delete existing template if needed. The fix prevents duplicates at multiple levels including protection against concurrent requests.
- **Implemented Admin-Only Template Deletion** - Added DELETE /api/workflow-templates/:id endpoint with requireAdmin() middleware and audit logging. Admin users can now delete workflow templates (not the original workflows) via trash icon button in Templates tab with confirmation dialog. Template deletion is fully decoupled from source workflows, ensuring templates can be removed without affecting active automations.
