...
- **Task Management**: Hierarchical sub-tasks, scheduling, dependencies, recurring tasks, bulk actions. Features a section-based task intake form, templated description generation, and automatic assignment. Product-to-task mapping supports automated task generation on lead conversion, with a robust task generation engine handling variable interpolation (including custom field merge tags), quantity modes, and idempotency. Task generation engine now expands bundles and packages to find product-level task mapping templates. Merge tags in task mapping templates are searchable and include both native fields (8) and all custom fields from Settings > Custom Fields. **Time Tracking**: Server-authoritative timer with dedicated endpoints (`POST /api/time-entries/start`, `POST /api/time-entries/stop`, `PATCH /api/time-entries/:id`, `DELETE /api/time-entries/:id`, `POST /api/time-entries/manual`). Duplicate guard prevents two simultaneous timers per user. Cross-tab sync via BroadcastChannel. Auto-cap service stops timers exceeding configurable threshold (default 12h). Startup migration caps stuck timers >24h. Time entry list in task detail with inline edit/delete. Meeting timer stop button in navbar header.
