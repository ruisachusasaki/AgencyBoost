...
- **Dashboard Widgets**: Customizable dashboard with drag-and-drop widget management, including 39 widgets across Client Management, Sales & Revenue, Tasks, Lead Management, HR & Team, and Calendar & Appointments categories. Features per-user widget layouts, team assignment filtering, add/remove UI, resizing, optimistic UI updates, real-time data updates, category-based filtering with color-coded badges (blue=Client Management, green=Sales, purple=Tasks, yellow=Leads, orange=HR & Team, cyan=Calendar & Appointments), and search functionality.
  ...
  - **Calendar & Appointments Widgets** (4): Today's Appointments, Upcoming Appointments (next 7 days), Appointment No-Shows (last 30 days), Overdue Appointments (past appointments needing status updates)
