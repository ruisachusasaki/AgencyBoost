...
import type { EmailTemplate, TemplateFolder } from "@shared/schema";
...
export default function EmailTemplates() {
  ...
  const deleteTemplateMutation = useMutation({
    mutationFn: (id: string) => apiRequest(`/api/email-templates/${id}`, { method: "DELETE" }),
    onSuccess: () => {
        ...
        {isLoading ? (
          ...
          <div className="col-span-full text-center py-12">
            ...
            <h3 className="text-lg font-medium text-muted-foreground mb-2">
              {searchTerm || selectedFolder !== "all" ? "No templates found" : "No email templates yet"}
            </h3>
            ...
            <Card key={template.id} className="group hover:shadow-md transition-shadow" data-testid={`card-template-${template.id}`}>
              <CardHeader className="pb-3">
                <div className="flex items-start justify-between">
                  <div className="space-y-1 flex-1">
                    ...
                    {template.subject && (
                      <p className="text-sm text-muted-foreground line-clamp-1" data-testid={`text-template-subject-${template.id}`}>
              ...
              <CardContent className="pt-0">
                <div className="space-y-3">
                  ...
                  {/* Content preview */}
                  <div className="text-sm text-muted-foreground line-clamp-3" data-testid={`text-template-content-${template.id}`}>
                    {template.content ? (
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            ...
            <div className="text-center p-4 bg-orange-50 dark:bg-orange-950/20 rounded-lg">
              <div className="text-2xl font-bold text-orange-600" data-testid="stat-usage">
                0
              </div>
