...
export default function LeadsByPipelineStageWidget({ userWidget, onRemove }: WidgetProps) {
  const { data, isLoading } = useQuery({
    queryKey: [`/api/dashboard-widgets/${userWidget.widgetType}/data`],
  });

  return (
    <Card data-testid="widget-leads-by-pipeline-stage" className="h-full flex flex-col">
              ...
              data.map((stage: any) => (
                ...
                >
                  <div className="flex items-center gap-3 flex-1">
                    ...
                    />
                    <p className="font-medium text-sm" data-testid={`stage-name-${stage.stageId}`}>
                      {stage.stageName}
                    </p>
                  </div>
                  <Badge variant="secondary" className="text-lg font-bold" data-testid={`stage-count-${stage.stageId}`}>
                    {stage.count}
                  </Badge>
                </div>
              ))
