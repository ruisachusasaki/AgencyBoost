...
} from "./chunk-NRRK27II.js";
import {
  db
} from "./chunk-23SKRIZB.js";
import {
  bundleProducts,
  businessProfile,
  clientBundles,
...
async function loadEmailTemplate() {
  ...
  try {
    const [setting] = await db.select().from(taskSettings).where(eq(taskSettings.settingKey, "proposal_email_template"));
    if (setting?.settingValue) {
      return { ...defaults, ...setting.settingValue };
    }
  } catch (e) {
  }
  return defaults;
...
Thank you!`,
  ...
  app.post("/api/quotes/:id/generate-token", requireAuth(), async (req, res) => {
    try {
      ...
      await db.update(quotes).set({ publicToken, expiresAt }).where(eq(quotes.id, id));
  ...
  app.get("/api/quotes/public/:token", async (req, res) => {
    try {
            ...
            for (const pi of pkgItemsList) {
              if (pi.itemType === "bundle" && pi.bundleId) {
                ...
                if (b) {
                  ...
                  const bundleItems = bps.filter((bp2) => bp2.name).map((bp2) => ({
                    name: bp2.name,
                    quantity: customQtys[bp2.productId] !== void 0 ? customQtys[bp2.productId] : bp2.quantity || 1
                  }));
      ...
      const servicesList = enrichedItems.map((item) => item.itemName).filter(Boolean).join(", ");
      const formatCurrency = (val) => val.toLocaleString("en-US", { minimumFractionDigits: 2, maximumFractionDigits: 2 });
      const effectiveDate = quote.sentAt ? new Date(quote.sentAt).toLocaleDateString("en-US", { year: "numeric", month: "long", day: "numeric" }) : (/* @__PURE__ */ new Date()).toLocaleDateString("en-US", { year: "numeric", month: "long", day: "numeric" });
      const resolvedTerms = quote.customAgreement ? { id: "custom", title: activeTerms[0]?.title || "Service Agreement", content: quote.customAgreement, version: activeTerms[0]?.version || 1, isActive: true } : activeTerms[0] ? { ...activeTerms[0] } : null;
  ...
  app.post("/api/quotes/public/:token/sign", async (req, res) => {
    try {
      ...
      const signerIpAddress = req.headers["x-forwarded-for"] ? Array.isArray(req.headers["x-forwarded-for"]) ? req.headers["x-forwarded-for"][0] : req.headers["x-forwarded-for"].split(",")[0].trim() : req.socket?.remoteAddress || "Unknown";
      const [quote] = await db.select().from(quotes).where(eq(quotes.publicToken, token));
      if (!quote) return res.status(404).json({ message: "Proposal not found" });
      if (quote.signedAt) {
  ...
  app.get("/api/settings/proposal-email-template", requireAuth(), async (req, res) => {
    try {
      ...
      if (setting?.settingValue) {
        res.json({ ...defaults, ...setting.settingValue });
      } else {
        res.json(defaults);
      }
    } catch (error) {
  ...
  app.put("/api/settings/proposal-email-template", requireAuth(), async (req, res) => {
    try {
      const templateSchema = z.object({
        ...
        introText: z.string().max(1e3).default("We've prepared a proposal for you. Please review the details, sign, and complete payment to get started."),
      ...
      const [existing] = await db.select().from(taskSettings).where(eq(taskSettings.settingKey, "proposal_email_template"));
      if (existing) {
        await db.update(taskSettings).set({ settingValue: templateData, updatedAt: /* @__PURE__ */ new Date() }).where(eq(taskSettings.settingKey, "proposal_email_template"));
      } else {
        await db.insert(taskSettings).values({
...
function applyMergeTags(text, clientName, quoteName) {
  return text.replace(/\{\{clientName\}\}/g, clientName).replace(/\{\{proposalName\}\}/g, quoteName);
}
function generateProposalEmailHtml(clientName, quoteName, proposalUrl, branding = {}, emailTpl) {
  ...
  const mt = (text) => escHtml(applyMergeTags(text, clientName, quoteName));
  return `<!DOCTYPE html>
<html>
<head>
  <meta charset="utf-8">
...
async function triggerQuoteFulfillment(quote, notificationService) {
  try {
    ...
    if (clientId) {
      try {
        const [onboardingConfig] = await db.select().from(clientOnboardingFormConfig).orderBy(desc(clientOnboardingFormConfig.updatedAt)).limit(1);
        if (onboardingConfig && Array.isArray(onboardingConfig.steps) && onboardingConfig.steps.length > 0) {
          ...
          if (existingClient && !existingClient.onboardingToken) {
            ...
            await db.update(clients).set({ onboardingToken: token, onboardingCompleted: false }).where(eq(clients.id, clientId));
