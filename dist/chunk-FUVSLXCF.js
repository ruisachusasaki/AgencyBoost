...
var DbStorage = class {
  ...
  async getHealthScoresFiltered(filters) {
    ...
    const sortColumn = sort === "weekStartDate" ? clientHealthScores.weekStartDate : sort === "clientName" ? clients.name : sort === "healthIndicator" ? clientHealthScores.healthIndicator : sort === "averageScore" ? clientHealthScores.averageScore : sort === "goals" ? clientHealthScores.goals : sort === "fulfillment" ? clientHealthScores.fulfillment : sort === "relationship" ? clientHealthScores.relationship : sort === "clientActions" ? clientHealthScores.clientActions : clientHealthScores.weekStartDate;
    if (sortOrder === "desc") {
  ...
  async updateTasksStatusForClient(clientId, targetStatus, filters) {
    const { db: db2 } = await import("./db-PWGDUHKZ.js");
    const { tasks: tasks2 } = await import("./schema-EEA2OOKK.js");
