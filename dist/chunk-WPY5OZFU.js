...
var NotificationService = class {
   ...
   */
  async notifyClientAssigned(userId, clientName, assignedBy, clientId) {
    await this.notify({
      userId,
   ...
   */
  async notifyTaskAssigned(userId, taskTitle, assignedBy, taskId, taskType = "task") {
    await this.notify({
      userId,
      type: "task_assigned",
      ...
      entityType: taskType,
      entityId: taskId,
      actionUrl: taskType === "task" ? `/tasks` : `/clients`,
      // Generic - can be improved
   ...
   */
  async notifyMentioned(userId, mentionedBy, contextType, contextId, message) {
   ...
   */
  async notifyMentionedInTask(userId, mentionedBy, taskId, taskName, commentContent) {
    const user = await this.storage.getStaffMember(mentionedBy);
    const mentionerName = user ? `${user.firstName} ${user.lastName}` : "Someone";
    ...
    const message = taskName ? `${mentionerName} mentioned you in "${taskName}": ${preview}` : `${mentionerName} mentioned you: ${preview}`;
    await this.notify({
      userId,
      type: "mentioned",
      title,
  ...
  shouldSendInApp(type, prefs) {
    ...
    return key ? Boolean(prefs[key]) : true;
  }
  /**
   ...
   */
  async deliverEmail(options, toEmail) {
   ...
   */
  async deliverSms(options, toPhone) {
  ...
  generateEmailHtml(options) {
    const appUrl = process.env.REPLIT_DEV_DOMAIN ? `https://${process.env.REPLIT_DEV_DOMAIN}` : "https://agencyflow.app";
    return `
<!DOCTYPE html>
<html>
<head>
...
<body>
   ...
   */
  getContextUrl(contextType, contextId) {
    const urlMapping = {
