...
var DbStorage = class {
  ...
  async seedSlackActions() {
    ...
    try {
      ...
      await db.update(automationActions).set({ category: "integration" }).where(and(
        eq(automationActions.type, "send_slack_message"),
  ...
  async updateTasksStatusForClient(clientId, targetStatus, filters) {
    const { db: db2 } = await import("./db-OSMGQACL.js");
    const { tasks: tasks2 } = await import("./schema-LT3BJ6BP.js");
